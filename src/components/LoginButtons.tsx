...
interface LoginButtonsProps {
  /** Where to redirect after login (hash fragment, e.g. '#/admin') */
  returnTo?: string
  /**
   * 'signIn' → "Sign in with GitHub/Google" (for login screens)
   * 'submit' → "Submit with GitHub/Google" (for leaderboard submission)
   * @default 'signIn'
   */
  variant?: 'signIn' | 'submit'
  /** Optional className for the container */
  className?: string
}

/**
 * Shared OAuth login buttons for GitHub and Google.
 */
export function LoginButtons({ returnTo, variant = 'signIn', className = '' }: LoginButtonsProps) {
  const { t } = useLanguage()

  const githubLabel = variant === 'submit' ? t(labels.signInWithGitHub) : t(labels.signInGitHub)
